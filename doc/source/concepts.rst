...
The central part of Marmalade-NG is the ledger module:
  ...
  - Manages all the accounts/tokens operation: mint/burn/transfer/sale
  - Calls the policies declared by tokens via modrefs.
  - Manage security of escrow accounts.
...
Every policy has a static dependency to the ledger:
  - For security: The policies needs to refer to the ledger capabilities.
  - Some policies needs some functions declared by the ledger for doing soem basic accounts management, like retrieving guards, create accounts.


Moreover Marmalade-NG include two (not mandatory to use) libraires:
  - std-policies: A set of pure functions which help to work with policies as strings.
  - util-policies: A set of functions which must be used for policies development only.
...
A policy hook can:
  - Check the allowed usage of a token and make the transaction fail if one the conditions is not respected by using a pact ``(enforce)``
...
Important: The only exception for the hook ``(enforce-sale-offer)``. The policy must:
  ...
  - return ``false`` if th policy accepts the sale but does not handle it.
  - return ``true`` **only** if the policy has chosen to handle the sale.

The ledger enforces that at least 1 policy has handled the sale.This is necessary to prevent the dramatic case when no policy wants to manage the sale.
...
The second syntax acts as a fallback and allow to define a common set of parameters for all tokens.
...
In this transaction, by convention the user include a ``marmalade_sale`` object in the data section
of the transaction:
...
The ledger move the token amount being sold to the escrow account.


Steps between 1 and 2:
~~~~~~~~~~~~~~~~~~~~~~
Some policies may require some extra step before ending the sale (eg: bidding for an auction sale).

But this a direct interaction between users and the policy and not managed by the Marmalade-NG core and the ledger.
...
If all the polices allow the withdrawal, the token is transferred back from the escrow account to the seller.
...
------------------------
This example shows how a sale defpact continuation can work and how the polices and the hooks ``(enforce-buy)`` and ``(enforce-settle)`` are chained according to their ranks.
